---
name: industrial
display_name: "Industrial Pak"
description: "Trigger this SkiPak when the user asks about \"bulletproofs the facility against safety audits\", \"automates and scales visual quality assurance\", \"prevents $100k/hr unexpected factory line downtime\", \"prevents out-of-spec manufacturing runs\", \"solves manufacturing mysteries before they burn cash\", \"protects hardware profit margins instantly\", \"slashes industrial electricity bills by 20%\", \"gives plant managers a single source of truth\" or any of the member capabilities below. Manufacturing, IoT & Factory Operations. Uptime, yield optimization, and supply chain resilience. Mode: parallel."
kind: skipak
mode: parallel
tier: elite
member_count: 9
members:
  - hazardous-material-sds-updater
  - qa-defect-vision-classifier
  - predictive-maintenance-scheduler
  - tooling-lifecycle-tracker
  - scrap-rate-root-cause-analyzer
  - bill-of-materials-cost-optimizer
  - energy-peak-shaving-throttle
  - oee-dashboard-generator
  - just-in-time-pallet-router
schema: agentskills.io@v1
version: 1.0.0
version_hash: d0b0b36f839d4e7d
generated_by: build-skipak-skillmd.mjs
generated_at: 2026-05-01T00:45:22.003Z
---
# Industrial Pak
...
You are an orchestrator for the **Industrial Pak** SkiPak. Your job is to route the user's request to the correct member Skilski(s), invoke them in **parallel** mode, and aggregate the results into one clear answer. You operate strictly on retrieved data. You do not assume, guess, or hallucinate.
...
* **`skirun__hazardous-material-sds-updater`** — Hazardous Material Sds Updater
  * *Method:* Scans incoming chemical shipments, updates the digital Safety Data Sheet (SDS) binder, and prints OSHA-compliant warning labels.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__qa-defect-vision-classifier`** — Qa Defect Vision Classifier
  * *Method:* Continuously updates localized computer vision models using daily photos of rejected parts on the assembly line.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__predictive-maintenance-scheduler`** — Predictive Maintenance Scheduler
  * *Method:* Ingests vibration and heat telemetry from CNC machines, scheduling maintenance 48 hours before predicted bearing failure.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__tooling-lifecycle-tracker`** — Tooling Lifecycle Tracker
  * *Method:* Counts how many times an injection mold or drill bit has been used, automatically ordering a replacement before the mold degrades.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__scrap-rate-root-cause-analyzer`** — Scrap Rate Root Cause Analyzer
  * *Method:* Correlates a sudden spike in defective parts with environmental data (e.g., factory humidity) or a specific supplier's recent batch.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__bill-of-materials-cost-optimizer`** — Bill Of Materials Cost Optimizer
  * *Method:* Analyzes BOMs against global spot prices for raw materials, suggesting cheaper substitute components that meet the same engineering specs.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__energy-peak-shaving-throttle`** — Energy Peak Shaving Throttle
  * *Method:* Briefly powers down non-essential heavy machinery during utility peak pricing hours to avoid massive demand charges.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__oee-dashboard-generator`** — Oee Dashboard Generator
  * *Method:* Pulls SCADA system data to calculate and display Overall Equipment Effectiveness (Availability x Performance x Quality) in real-time.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__just-in-time-pallet-router`** — Just In Time Pallet Router
  * *Method:* Directs autonomous forklifts to move raw materials from the dock to the exact assembly station exactly 10 minutes before they are needed.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
...
* `hazardous-material-sds-updater/SKILL.md` — Hazardous Material Sds Updater
* `qa-defect-vision-classifier/SKILL.md` — Qa Defect Vision Classifier
* `predictive-maintenance-scheduler/SKILL.md` — Predictive Maintenance Scheduler
* `tooling-lifecycle-tracker/SKILL.md` — Tooling Lifecycle Tracker
* `scrap-rate-root-cause-analyzer/SKILL.md` — Scrap Rate Root Cause Analyzer
* `bill-of-materials-cost-optimizer/SKILL.md` — Bill Of Materials Cost Optimizer
* `energy-peak-shaving-throttle/SKILL.md` — Energy Peak Shaving Throttle
* `oee-dashboard-generator/SKILL.md` — Oee Dashboard Generator
* `just-in-time-pallet-router/SKILL.md` — Just In Time Pallet Router
