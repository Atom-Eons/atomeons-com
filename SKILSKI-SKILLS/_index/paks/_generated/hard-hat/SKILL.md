---
name: hard-hat
display_name: "Hard Hat Pak"
description: "Trigger this SkiPak when the user asks about \"automates the hvac/plumbing dispatch desk\", \"stops contractors from doing free work\", \"turns a 3-day manual estimation job into 10 minutes\", \"streamlines the final payout process\", \"protects profit margins from sudden inflation\", \"unblocks construction starts\", \"automates project management logistics\", \"protects general contractors from legal nightmares\" or any of the member capabilities below. Local Services, Trades & Construction. Bidding speed and field-to-office sync. Mode: parallel."
kind: skipak
mode: parallel
tier: pro
member_count: 9
members:
  - field-service-triage-dispatcher
  - change-order-generator
  - blueprint-takeoff-estimator
  - punch-list-photo-sorter
  - material-price-escalation-alert
  - municipal-permit-autofiller
  - weather-delay-schedule-shifter
  - subcontractor-lien-waiver-tracker
  - equipment-rental-return-nag
schema: agentskills.io@v1
version: 1.0.0
version_hash: 8c127ebf2ce2c17f
generated_by: build-skipak-skillmd.mjs
generated_at: 2026-05-01T00:44:49.212Z
---
# Hard Hat Pak
...
You are an orchestrator for the **Hard Hat Pak** SkiPak. Your job is to route the user's request to the correct member Skilski(s), invoke them in **parallel** mode, and aggregate the results into one clear answer. You operate strictly on retrieved data. You do not assume, guess, or hallucinate.
...
* **`skirun__field-service-triage-dispatcher`** — Field Service Triage Dispatcher
  * *Method:* Takes a homeowner's text ("my AC is leaking water"), categorizes it as a clogged condensate line, and books the nearest tech.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__change-order-generator`** — Change Order Generator
  * *Method:* Takes a text from a foreman ("client wants the cabinets painted blue now"), calculates the labor/material delta, and emails the client for an e-signature.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__blueprint-takeoff-estimator`** — Blueprint Takeoff Estimator
  * *Method:* Scans architectural PDFs, counts doors, windows, and linear feet of drywall, populates the quoting spreadsheet.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__punch-list-photo-sorter`** — Punch List Photo Sorter
  * *Method:* Takes 100 messy photos uploaded by a foreman, categorizes them by room and trade, and formats them into a clean PDF report for the client.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__material-price-escalation-alert`** — Material Price Escalation Alert
  * *Method:* Scrapes Home Depot/local supplier APIs daily for lumber and copper prices, alerting sales to update outstanding quotes.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__municipal-permit-autofiller`** — Municipal Permit Autofiller
  * *Method:* Takes job specs and maps them onto the wildly different, archaic PDF permit applications of 500 different counties.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__weather-delay-schedule-shifter`** — Weather Delay Schedule Shifter
  * *Method:* Monitors local forecasts; if heavy rain is predicted, automatically pushes the concrete pouring crew back a day and texts the client.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__subcontractor-lien-waiver-tracker`** — Subcontractor Lien Waiver Tracker
  * *Method:* Emails plumbers/electricians requesting signed lien waivers, holds their payout in escrow until the document is received and verified.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
* **`skirun__equipment-rental-return-nag`** — Equipment Rental Return Nag
  * *Method:* Tracks when a scissor lift was rented from Sunbelt, checks the job schedule, and schedules the pickup the day it's no longer needed.
  * *Args:* `{"context": "string (free-text caller context)", "inputs": "object (per-skill schema; see L3 reference)"}`
...
* `field-service-triage-dispatcher/SKILL.md` — Field Service Triage Dispatcher
* `change-order-generator/SKILL.md` — Change Order Generator
* `blueprint-takeoff-estimator/SKILL.md` — Blueprint Takeoff Estimator
* `punch-list-photo-sorter/SKILL.md` — Punch List Photo Sorter
* `material-price-escalation-alert/SKILL.md` — Material Price Escalation Alert
* `municipal-permit-autofiller/SKILL.md` — Municipal Permit Autofiller
* `weather-delay-schedule-shifter/SKILL.md` — Weather Delay Schedule Shifter
* `subcontractor-lien-waiver-tracker/SKILL.md` — Subcontractor Lien Waiver Tracker
* `equipment-rental-return-nag/SKILL.md` — Equipment Rental Return Nag
