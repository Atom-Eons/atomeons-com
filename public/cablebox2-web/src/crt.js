...
let attachedVideo = null, attachedCanvas = null;
...
function tick() {
  ...
  if (attachedVideo && attachedVideo.readyState >= 2) {
    // texImage2D throws SecurityError if the video is a tainting cross-origin source
    // (the native-HLS path on browsers without MSE). Previously that throw escaped
    // BEFORE the requestAnimationFrame below, so one bad frame killed the CRT loop for
    // the whole session. Now a tainted source disables the filter and leaves the plain
    // picture visible instead of freezing it.
    try {
      ...
      console.warn('CRT disabled: video frame is not readable by WebGL', err?.name || err);
      running = false;
      if (attachedCanvas) attachedCanvas.style.display = 'none';
      return;
    }
  }
...
export function startCrt(videoEl, canvasEl) {
  ...
  canvasEl.style.display = '';   // clear a previous taint-disable
  resizeCrt(canvasEl);
  running = true;
