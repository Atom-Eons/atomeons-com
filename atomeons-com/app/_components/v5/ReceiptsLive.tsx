      ...
      <div className="mx-auto w-full max-w-7xl px-6">
        <div className="grid gap-16 lg:grid-cols-[1fr_1.2fr] lg:items-start">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-[#1A2225] bg-[#0A0F11] shadow-[0_0_120px_-40px_rgba(255,122,26,0.3)]">
            ...
            {/* column headers */}
            <div className="grid grid-cols-[80px_60px_80px_80px_60px_50px] gap-3 border-b border-[#1A2225] px-5 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-[#6B7779]">
              <span>ts</span>
              <span>dept</span>
              <span>tool</span>
            ...
            <div className="min-h-[360px] divide-y divide-[#1A2225]">
              {visible.map((r, idx) => (
                <div
                  key={`${r.ts}-${idx}`}
                  className="grid grid-cols-[80px_60px_80px_80px_60px_50px] items-center gap-3 px-5 py-2.5 font-mono text-xs"
                  style={{
                    opacity: 1 - idx * 0.08,
                    animation:
